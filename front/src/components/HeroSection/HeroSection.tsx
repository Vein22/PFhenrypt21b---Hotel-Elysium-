...
const HeroSection = () => {
  ...
  return (
    <section
      className="px-4 bg-cover bg-center h-screen flex items-center justify-center text-center"
      style={{ backgroundImage: "url(/fondo3.png)" }}
    >
      <div className="bg-black bg-opacity-50 rounded-lg max-w-4xl mx-auto p-4 sm:p-8">
        <h2
          className="text-[1.5rem] sm:text-[2rem] lg:text-[2.5rem] text-white"
        >
          Hotel y Resort de Lujo
        </h2>
        <h1
          className="text-white text-[2rem] sm:text-[3rem] lg:text-[4rem] uppercase my-4 sm:my-6"
        >
          Vive una Experiencia Única en ELYSIUM
        </h1>

        <motion.button
          className="bg-mostaza border-mostaza uppercase text-white py-2 px-4 sm:py-3 sm:px-6 transition-all"
          whileHover={{ scale: 1.05 }}
          onClick={() => setShowMessage(!showMessage)}
        >
          Saber más
        ...
        {showMessage && (
          <motion.div
            className="mt-6 p-4 text-base sm:text-xl rounded-lg bg-white shadow-lg border-4 border-marronclaro"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            transition={{ duration: 0.5 }}
          >
            <span className="text-grisOscuro font-secondary">
              El hotel perfecto. Si buscas unas vacaciones perfectas y recuerdos
              inolvidables, estás en el lugar indicado. Planifiquemos una
              estancia a tu medida, con los mejores Servicios, Atención en
              Habitación, Spa, Gimnasio, Peluquería Profesional, Desayunos
              Buffet, la mejor Seguridad y mucho más.
            </span>
          </motion.div>
        )}
      </div>
